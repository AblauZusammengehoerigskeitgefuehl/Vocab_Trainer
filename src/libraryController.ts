...
export class LibraryController {
  ...
  datasetsView() {
    ...
    const datasets = query
      ...
      : this.state.datasets;
    if (!this.state.activeLanguagePairId) return `<p>Create a pair first.</p>`;
    if (!this.state.datasets.length) return `<p>No datasets saved for this pair yet.</p>`;
    if (!datasets.length) return `<p>No datasets match your search.</p>`;
    return datasets.map((dataset) => this.datasetRow(dataset)).join("");
  }

  noteEditorView() {
    if (!this.state.activeLanguagePairId) return this.newPairView();
    return `
      <div class="section-heading">
        <div>
  ...
  renderDatasets() {
    ...
    if (list) list.innerHTML = this.datasetsView();
  }

  renderPreview() {
    const preview = this.app.querySelector("[data-library-preview]");
    if (preview) preview.innerHTML = this.state.preview ? this.importPreviewView() : "";
  }

  renderEditor() {
    const editor = this.app.querySelector("[data-library-editor]");
    if (editor) editor.innerHTML = this.noteEditorView();
  }
  ...
  syncDatasetsPanel() {
    ...
    if (body) body.hidden = !this.state.libraryDatasetsOpen;
    if (toggle) {
      ...
      if (status) status.textContent = this.state.libraryDatasetsOpen ? "Hide" : "Show";
    }
  }
  ...
  async handleAction(action, target) {
    ...
    if (action === "reparse-invalid") {
      ...
      if (!this.state.preview) await this.callbacks.refreshAll();
      this.renderLive();
      return true;
    }
    ...
    if (action === "confirm-import") {
      ...
      await this.callbacks.refreshAll();
      if (this.state.editDatasetId) this.callbacks.loadExistingDataset(this.state.editDatasetId);
      this.renderLive();
      return true;
    }
    ...
    if (action === "save-existing") {
      ...
      await this.callbacks.refreshAll();
      if (editedId && this.state.datasets.some((dataset) => dataset.id === editedId)) this.callbacks.loadExistingDataset(editedId);
      this.renderLive();
      return true;
    }
    return false;
