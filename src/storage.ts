...
export function openDb() {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains("languagePairs")) db.createObjectStore("languagePairs", { keyPath: "id" });
      ...
      if (!db.objectStoreNames.contains("quizSessions")) db.createObjectStore("quizSessions", { keyPath: "id" });
...
export class VocabularyRepository {
  ...
  async getByDatasetIds(datasetIds) {
    if (datasetIds === "all") return this.getAll();
...
function sanitizeEntry(entry) {
  ...
  if (clean.typeCode === 2) {
    ...
    if (raw) clean.conjugation = { raw };
    else delete clean.conjugation;
    if (!existingConjugation && legacyDetails) delete clean.details;
  } else {
    delete clean.conjugation;
  }
  if (!clean.details?.trim()) delete clean.details;
  return clean;
}
