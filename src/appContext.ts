...
import {
  DatasetRepository,
  DuplicateConflictRepository,
  InvalidLineRepository,
...
export class AppContext {
  ...
  async refreshAll() {
    ...
    state.languagePairs = await this.languagePairsRepo.list();
    if (!state.activeLanguagePairId) state.activeLanguagePairId = state.settings.activeLanguagePairId ?? null;
    ...
    if (!state.activeLanguagePairId && state.languagePairs[0]) state.activeLanguagePairId = state.languagePairs[0].id;
    ...
    if (state.selectedDatasetId && !activeDatasetIds.has(state.selectedDatasetId)) state.selectedDatasetId = null;
    if (!state.selectedDatasetId && state.datasets[0]) state.selectedDatasetId = state.datasets[0].id;
