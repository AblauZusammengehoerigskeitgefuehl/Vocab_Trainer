...
export class AppShellRenderer {
  ...
  renderShell() {
    if (this.app.dataset.mode === "shell" && this.app.querySelector("[data-app-content]")) return;
  ...
  updateNavState() {
    ...
    if (pairNav) pairNav.innerHTML = this.languagePairNav();
  }

  updateMessageView() {
    const message = this.app.querySelector("[data-message]");
    if (message) message.innerHTML = this.state.message ? `<div class="notice">${this.callbacks.escapeHtml(this.state.message)}</div>` : "";
  }
  ...
  renderRouteView() {
    ...
    if (!routeChanged || !activeView.dataset.rendered) {
      ...
      if (!routeChanged) {
        this.app.querySelectorAll("[data-view]").forEach((view) => {
          if (view !== activeView) delete view.dataset.rendered;
        });
      }
    }
  ...
  languagePairNav() {
    if (!this.state.languagePairs.length) return "";
    return `
      <div class="pair-nav">
