// @ts-nocheck
import { conjugationText, createEntry, nowIso, uid, } from "./core.js";
...
export class LibraryDataController {
    ...
    async confirmImport() {
        if (!this.state.preview || this.previewHasBlockingErrors())
            return;
    ...
    async confirmEditedPreview() {
        ...
        for (const id of this.state.deletedEntryIds)
    ...
    async skipInvalid(id) {
        if (this.state.preview)
            this.state.preview.invalidLines = this.state.preview.invalidLines.filter((line) => line.id !== id);
        else
            await this.invalidRepo.delete(id);
    }
    keepConflict(id) {
    ...
    discardIncomingEntry(entryId) {
        ...
        this.state.preview.duplicateConflicts = this.state.preview.duplicateConflicts.filter((conflict) => (conflict.incomingEntry.id !== entryId
    ...
    async saveExisting() {
        ...
        const entries = validRows.map((row) => {
            if (row.id)
                return this.normalizeEditedEntry({ ...row, datasetId: dataset.id, updatedAt: time });
    ...
    removeExistingRow(index) {
        ...
        if (row?.id)
            this.state.deletedEntryIds.push(row.id);
    }
    ...
    sourceTypeFromFile(file) {
        ...
        if (["pdf", "txt", "csv", "tsv", "json"].includes(ext))
            return ext;
        return "txt";
    }
}
