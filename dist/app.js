// @ts-nocheck
import { LANGUAGE_REGISTRY, TYPE_CODE_MAP, TYPE_OPTIONS, applySettingsToDocument, canonicalPairId, languageByName, languageLabel, nowIso, pairLabel, } from "./core.js";
...
const { app, state, datasetsRepo, languagePairsRepo, settingsRepo, } = context;
...
function syncDatasetControls(scope, config) {
    ...
    if (count)
        count.textContent = `${selectedIds.length} of ${state.datasets.length} selected`;
...
async function handleAction(action, target) {
    ...
    if (state.route === "notes" && await handleLibraryAction(action, target))
        return;
    let handled = true;
    if (action === "clear-preview")
        state.preview = null;
    else if (action === "skip-preview-invalid" && state.preview)
        state.preview.invalidLines = [];
    else if (action === "reparse-invalid")
        await reparseInvalid(target.dataset.invalidId);
    else if (action === "skip-invalid")
        await skipInvalid(target.dataset.invalidId);
    else if (action === "keep-conflict")
        keepConflict(target.dataset.conflictId);
    else if (action === "discard-conflict")
        discardConflict(target.dataset.conflictId);
    else if (action === "keep-all-conflicts")
        keepAllConflicts();
    else if (action === "discard-all-conflicts")
        discardAllConflicts();
    else if (action === "keep-all-warning-conflicts")
        keepAllWarningConflicts();
    else if (action === "discard-all-warning-conflicts")
        discardAllWarningConflicts();
    else if (action === "confirm-import")
        await confirmImport();
    else if (action === "add-manual-row")
        state.manualRows.push(emptyManualRow());
    else if (action === "remove-manual-row")
        state.manualRows.splice(Number(target.dataset.rowIndex), 1);
    else if (action === "add-existing-row")
        state.editRows.push(emptyEditRow(state.editDatasetId));
    else if (action === "remove-existing-row")
        removeExistingRow(Number(target.dataset.rowIndex));
    else if (action === "save-manual")
        await saveManual();
    else if (action === "save-existing")
        await saveExisting();
    else if (action === "delete-dataset")
        await deleteDataset(target.dataset.datasetId);
    else if (action === "start-quiz")
        await startQuiz();
    else if (action === "finish-quiz")
        await finishQuiz();
    else if (action === "reveal")
        await revealCurrent();
    else if (action === "verb-translation-plus" || action === "verb-translation-minus")
        await markVerbTranslation(action.endsWith("plus") ? "+" : "-");
    else if (action === "verb-details-plus" || action === "verb-details-minus")
        await markVerbDetails(action.endsWith("plus") ? "+" : "-");
    else if (action === "quiz-plus" || action === "quiz-minus")
        await scoreCurrent(action.endsWith("plus") ? "+" : "-");
...
async function onChange(event) {
    ...
    if (state.route === "notes" && target.matches("[data-manual-field], [data-existing-field], [data-manual-title], [data-existing-title], [data-preview-title], [data-invalid-edit], [data-library-dataset-search], [data-pair-language]")) {
        if (target.matches("[data-manual-field]"))
            updateManualField(target);
        if (target.matches("[data-existing-field]"))
            updateExistingField(target);
        if (target.matches("[data-manual-title]"))
            state.manualTitle = target.value;
        if (target.matches("[data-existing-title]"))
            state.editTitle = target.value;
        if (target.matches("[data-preview-title]") && state.preview)
            state.preview.dataset.title = target.value;
...
function onInput(event) {
    ...
    if (target.matches("[data-rename-dataset]"))
        debounceRename(target.dataset.renameDataset, target.value);
    if (target.matches("[data-setting]"))
        updateSettingDraft(target);
}
...
function updateControl(target) {
    for (const scope of ["learn", "quiz"]) {
        ...
        if (target.matches(`[data-${scope}-type]`)) {
            ...
            if (target.checked && !config.typeFilters.includes(value))
                config.typeFilters.push(value);
            if (!target.checked)
                config.typeFilters = config.typeFilters.filter((item) => item !== value);
        }
    }
}
...
function updatePairLanguageDraft(target) {
    if (target.dataset.pairLanguage === "a")
        state.pairLanguageSearchA = target.value;
    if (target.dataset.pairLanguage === "b")
        state.pairLanguageSearchB = target.value;
}
let renameTimer;
...
function onKeydown(event) {
    if (state.route !== "quiz" || !state.settings.keyboardShortcutsEnabled)
        return;
    ...
    if (event.key === "ArrowRight")
        handleAction("quiz-plus", {});
    if (event.key === "ArrowLeft")
        handleAction("quiz-minus", {});
}
...
async function switchLanguagePair(pairId, options = {}) {
    if (!pairId || state.activeLanguagePairId === pairId && !options.skipRefresh)
        return;
...
function formatConjugation(value) {
    ...
    raw.split(";").forEach((part) => {
        ...
        if (match)
            byTense.set(match[1].toLowerCase(), match[2].trim());
    });
    if (!byTense.size)
