// @ts-nocheck
import { canShowVerbDetails, dedupeEligibleEntries, entryKey, makeQuizSession, nowIso, quizItemForEntry, } from "./core.js";
export class QuizSessionController {
    ...
    async updateQueue() {
        ...
        const queue = eligible.map((entry) => {
            ...
            if (!existing)
                return quizItemForEntry(entry, this.state.quizConfig.direction);
            return {
                ...existing,
    ...
    async finish() {
        if (!confirm("Finish quiz? Current session scores will reset."))
            return;
    ...
    async finalizeCurrentVerbScore() {
        const item = this.currentItem();
        if (!item?.verbTranslationResult || !item.verbDetailsResult)
            return;
    ...
    async applyFinalScore(item, finalResult) {
        ...
        if (nextScore >= 5 && entry) {
            ...
            for (const candidate of updatedEntries)
                await this.vocabRepo.updateEntry(candidate);
    ...
    shuffleQuizQueue(queue, previousOrder = []) {
        ...
        for (let attempt = 0; attempt < 8; attempt += 1) {
            ...
            if (!this.sameOrder(shuffled, previousOrder))
                return shuffled;
        }
