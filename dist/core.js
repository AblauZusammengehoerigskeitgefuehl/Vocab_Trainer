...
export function createEntry({ datasetId, typeCode = 6, spanish, english, details = "", conjugation = "" }) {
    ...
    if (safeType === 2 && conjugationText)
        entry.conjugation = { raw: conjugationText };
    return entry;
}
export class VocabularyParser {
    ...
    parseHeader(line, datasetId, languagePairs = []) {
        ...
        if (!source)
            return { valid: false, invalidLine: invalidLine(`Unknown language code "${parts[1]}". Create pairs using the language picker.`) };
        if (!target)
            return { valid: false, invalidLine: invalidLine(`Unknown language code "${parts[2]}". Create pairs using the language picker.`) };
        if (source.code === target.code)
            return { valid: false, invalidLine: invalidLine("Source and target languages must be different.") };
    ...
    parseLine(line, datasetId, lineNumber = 1) {
        ...
        if (trimmed.includes("&"))
            return invalid("Ampersands are not supported. Use the details field for a source-language usage sentence.");
        const parts = trimmed.split("/").map((field) => field.trim());
        if (parts.length < 3)
            return invalid("Missing required fields. Use: type / source / target / usage sentence");
        if (/^[-*•]/.test(trimmed))
            return invalid("Bullets are not valid vocabulary lines.");
        if (/^\d+\.\s/.test(trimmed))
            return invalid("Numbered lines are not valid. Use a numeric type followed by /.");
        ...
        if (!parts[1])
            return invalid("Missing source field.");
        if (!parts[2])
            return invalid("Missing target field.");
        ...
        if (typeCode === 2 && !conjugationField)
            return invalid("Missing verb conjugation field.");
        if (!detailsField)
            return invalid("Missing usage sentence field.");
        return createEntry({
            datasetId,
            typeCode,
            spanish: parts[1],
...
function shuffleRandom(entries) {
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
        ...
        if (!shuffled.every((entry, index) => entry.id === originalIds[index]))
            return shuffled;
    }
...
export function filterEntries(entries, config) {
    ...
    let visible = entries.filter((entry) => {
        if (!typeFilters.has(entry.typeCode))
            return false;
        if (!config.includeMastered && entry.masteryCount >= 5)
            return false;
        if (!query)
            return true;
